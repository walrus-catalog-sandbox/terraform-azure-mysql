locals {
  ...
  hosts = [
    ...
    format("%s", azurerm_mysql_flexible_server.primary.fqdn) :
    format("%s.%s", azurerm_mysql_flexible_server.primary.name, var.infrastructure.domain_suffix)
  ]
  hosts_readonly = local.architecture == "replication" ? flatten([
    var.infrastructure.domain_suffix == null ?
    azurerm_mysql_flexible_server.secondary[*].fqdn :
    [format("%s.%s", azurerm_mysql_flexible_server.secondary[0].name, var.infrastructure.domain_suffix)]
  ]) : []
